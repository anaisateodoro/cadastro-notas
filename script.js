    ...
    if (list.length != 0) {
        for (var i = 0; i < list.length; i++)
            ...
            <tr>
                ...
                <td>
                    <a href="#" onclick="editItem(${i})">Editar | </a>
                    <a href="#" onclick="deleteItem(${i})">Deletar</a>
...
function editItem(matricula) {
    storage = JSON.parse(sessionStorage.ALUNOS);
    let aluno = storage[matricula];
    focusedId = matricula;
...
function deleteItem(matricula) {
    storage = JSON.parse(sessionStorage.ALUNOS);
    storage.splice(matricula, 1);
    sessionStorage.setItem("ALUNOS", JSON.stringify(storage));
    makeTable(storage, alunos);
}
